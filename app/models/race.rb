class Race
	...
	has_many :entrants, dependent: :delete, foreign_key: "race._id", order: [ :secs.asc, :bib.asc ]
	...
	def create_entrant racer
		...
		events.each { |e| entrant.send( "#{e.name}=" , e ) }
		if entrant.validate
			entrant.inc( bib: 1 )
			entrant.save
			self.next_bib 
	...
	def self.upcoming_available_to racer
		...
		Race.where( :_id => { :$nin => upcoming_race_ids }, :date => { :$gt => Date.current } )
	end
end